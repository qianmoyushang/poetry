...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getArticleTag() {
        console.log("getTag==148==",this.addData.tags);
        let theMap = {};
      ...
      canPublish() {
        if (this.addData.title.length === 0) {
          ...
          if (this.addData.content && this.addData.content.length > 10) {
            return true;
          } else {
            this.$Message.error("古诗内容长度不够10");
            return false;
          }
        }
      },
      handlePublish() {
        if (this.canPublish()) {
          ...
          this.$http.request({
            ...
            this.$Notice.success({
              title: "保存成功",
              desc: "古诗《" + this.addData.title + "》修改成功"
            });
      ...
      getData() {
        ...
        this.$http.request({
          ...
          console.log("adddata id 275=="+JSON.stringify(res));
          console.log("adddata id 274=="+this.addData.id);
          // console.log("adddata=="+this.addData.title);
          // console.log("categorys===="+this.addData.categorys);
          ...
          if (this.addData.categorys) {
            this.addData.categorys = this.addData.categorys.map(item => {
              console.log("item===284=="+item);

              if (cmap[item]) {
                // console.log("cmap-item=="+cmap[item]);
                if (cmap[item].children) {
                  // console.log("cmap-children=="+cmap[item].children);
                  return {
                    id: item,
                    color: "green"
                  };
          ...
          console.log("tmap_tags===305="+this.addData.tags);
          console.log("tmap====306=="+JSON.stringify(tmap));

          if (this.addData.tags) {
            this.addData.tags = this.addData.tags.map(item => {
              console.log("0000000=="+item);
              console.log("1111111=="+tmap[item]);

              if (tmap[item]) {
                console.log("item-children1=="+tmap[item]);
                if (tmap[item].children) {
                  console.log("item-children2=="+tmap[item].children);
                  return {
                    id: item,
                    color: "green"
                  };
          ...
          // console.log("content=="+this.addData.content);
          // this.$refs.editor.setHtml(this.addData.content);

          this.getArticleCategory();
          console.log("content2===334="+this.addData.content);
          this.getArticleTag();
          console.log("content3=="+this.addData.content);
          this.$Notice.success({
            title: "获取古诗正文"
          });
        });
